const HOMES_LANGS = {
    "care-home": {
        hero: "/home/services/care-homes.webp",
        subtitle: "CARE HOMES",
        about: {
            para1: "Senkoun Healthcare Wakering Ltd presents a visionary plan to\n" +
                "                        establish Joseph Lodge in Little Wakering Southend-on-Sea, With a\n" +
                "                        strong focus on delivering high-quality healthcare services and\n" +
                "                        addressing the pressing needs of the ageing population, Joseph Lodge\n" +
                "                        aims to become a trusted and preferred choice for elderly care in\n" +
                "                        the region.",
            para2: "Joseph Lodge business plan showcases a strong vision for providing\n" +
                "                        exceptional residential care in Southend-on-Sea. With a focus on\n" +
                "                        innovation, compliance, and customer-centric services, we aim to\n" +
                "                        become a leader in the industry. By strategically acquiring\n" +
                "                        additional properties in the future, we will expand our reach and\n" +
                "                        positively impact the lives of more elderly individuals in need of\n" +
                "                        specialized care.",
            para3: "This expansion strategy is driven by the increasing demand for\n" +
                "                        high-quality residential and or nursing care services and our\n" +
                "                        commitment to meeting the evolving needs of the ageing population.\n" +
                "                        We will achieve this through a six-step process"
        },
        lifeAt: "Joseph Lodge is a care home for the elderly providing personal care to people with age related issues including dementia. The home is set in a beautiful rural location a short distance from the outskirts of Southend",
        howWeCare: "Joseph Lodge is a care home for the elderly providing personal care to people with age related issues including dementia. The home is set in a beautiful rural location a short distance from the outskirts of Southend.",
        testimonials: [
            {
                quote: "This home is amazing interaction with the residents is excellent. Always something for them to do. Very well cared for and a pleasure to visit . My Aunt had a few difficulty settling in. But manager was excellent how she handled my aunts difficulties. We as a family know she is safe cared for and we are welcolm when we visit . Thank you staff you are amazing",
                author: "Niece of Resident",
            },
            {
                quote: "Joseph Lodge is a very well organised home. Staff are very caring and friendly and make you feel welcome. The activities supplied by Heather are amazing, it's certainly noticed when she is not there. The facilities are very good - there us a hair salon which can be used by the family to wash their relatives hair as well as a hair dresser every two weeks.",
                author: "Daughter of Resident",
            },
            {
                quote:
                "My Mum went to Joseph Lodge on 12th August from hospital as unable to care for herself any longer. The staff are brilliant, nothing is too much trouble. They keep me informed and give a comprehensive update whenever myself or family visit. The home has recently opened after refurbishment and is very clean and the decor is pleasing.\n" +
                    "At present activities, hair salons, etc are yet to be introduced as they are still filling up with residents.\n",
                author: "Daughter of Resident",
            }
        ]
    },
    "supported-living": {
        hero: "/home/services/care-homes.webp",

        subtitle: "SUPPORTED LIVING",
        about: {
            para1: "",
            para2: "",
            para3: ""
        },
        lifeAt: "Supported Living at Joseph Lodge offers a nurturing environment where individuals with varying needs can thrive. Our dedicated team provides personalized support, empowering residents to lead fulfilling lives while enjoying the comforts of home.",
        howWeCare: "At Joseph Lodge Supported Living, we prioritize the unique needs of each resident. Our compassionate staff work closely with individuals to create tailored care plans that promote independence, well-being, and a sense of community.",
        testimonials: [
            {
                quote: "Living at Joseph Lodge has been a transformative experience for me. The support I receive allows me to pursue my interests and live independently while knowing that help is always available when I need it.",
                author: "RESIDENT",
            },
            {
                quote: "The team at Joseph Lodge Supported Living has been incredible. They understand my needs and provide the right balance of support and freedom. I feel valued and respected here.",
                author: "RESIDENT",
            }
        ]
    },
    "domiciliary-care": {
        hero: "/home/services/care-homes.webp",
        subtitle: "DOMICILIARY CARE",
    }
}


export default HOMES_LANGS;