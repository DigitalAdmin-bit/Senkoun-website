...
import type {IJobResponse, THearAboutVacancyOption} from "@/lib/apis/jobs";
...
export default function RightSide({selectedJob}: RightSideProps) {
    const [activeTab, setActiveTab] = useState<"description" | "application">(
        "description",
    );
    ...
    const createJobApplicationSchema = () => {
        const baseSchema = {
            ...
            hear_about_vacancy: z.enum(
                hearAboutOptions as [
                    THearAboutVacancyOption,
                    ...THearAboutVacancyOption[],
                ],
            ),
            resume: z
        ...
        if (selectedJob?.questions) {
            selectedJob.questions.forEach((question, index) => {
                ...
                if (question.questionType === "checkbox") {
                    // Checkbox can yield a boolean (single option), an array of
                    // selected option strings (multiple options), or undefined.
                    const checkboxValue = z.union([
                        z.boolean(),
                        z.array(z.string()),
                        z.string(),
                    ]);
                    questionFields[fieldKey] = question.required
                        ? checkboxValue.refine(
                              (value) => {
                        ...
                        : checkboxValue.optional();
                    return;
                }

                if (question.required) {
                    questionFields[fieldKey] = z
                        .string()
    ...
    const onSubmit = async (data: JobApplicationFormData) => {
        ...
        try {
            ...
            const getAnswerString = (value: unknown): string => {
                if (Array.isArray(value)) {
                    return value.join(", ");
                }
                if (typeof value === "boolean") {
            ...
            const responses =
                selectedJob.questions?.map((question, index) => ({
                    ...
                    answer: getAnswerString((data as any)[`question_${index}`]),
            ...
            router.push(
                `/careers/openings/success?email=${encodeURIComponent(data.email)}`,
            );
                    ...
                    £{selectedJob.salary.amount}/{selectedJob.salary.period}
                      ...
                      {tag.text}
                        ...
                        {activeTab === "description" ? (
                            <>
                                {/* Job Description */}
                                <div className="mb-8 text-[#64565A]">
                            ...
                            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-lg font-semibold text-[#64565A]">
                                        Personal Information
                                    </h3>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {errors.hear_about_vacancy && (
                                            <p className="text-sm text-red-600">
                                                {errors.hear_about_vacancy.message}
                                            </p>
                                        )}
                                    </div>
                                </div>
                                                        ...
                                                        <div className="space-y-2">
                                                            {question.options
                                                                ...
                                                                .map((option, optIndex) => (
                                                                    ...
                                                                    >
                                                                        <input
                                                                            id={`question_${index}_${optIndex}`}
                                                                            type="checkbox"
                                                                            value={option.trim()}
                                                                            {...register(
                                                                                `question_${index}` as any,
                                                                            )}
                                                                            className="h-4 w-4 rounded border-gray-400 text-[#B8853A] focus:ring-2 focus:ring-[#B8853A]"
                                                                        />
                                                                        <label
                                                                            htmlFor={`question_${index}_${optIndex}`}
